...
echo "======================================================="
echo ""
echo "  What would you like to do with rd_bios_set.zip?"
echo ""
echo "    E - Use existing rd_bios_set.zip as-is (skip to Step 8)"
echo "    A - Add to existing rd_bios_set.zip from a URL or local directory"
...
if [[ "$BIOS_SET_CHOICE" =~ ^[Ee]$ ]]; then
    ...
    if [[ "$BIOS_SET_CHOICE" =~ ^[Aa]$ ]]; then
        ...
        if [[ ! -f "$EXISTING_ZIP" ]]; then
            echo ""
            echo "  NOTE: No existing rd_bios_set.zip found. A new one will be created."
        else
            echo ""
            echo "  Adding to existing rd_bios_set.zip."
    ...
    if [[ "$ZIP_MODE" == "overwrite" && -f "$EXISTING_ZIP" ]]; then
        echo ""
        echo "  ****WARNING****"
        echo "  This will overwrite the existing rd_bios_set.zip."
...
def is_unverified_match(entry):
    """File matched manifest by filename and was found in archive, but RetroDECK
...
if copied_unverified:
    print(f"  Copied (unverified): {copied_unverified} file(s) — filename matched manifest; no expected MD5 in RetroDECK manifests.")
if skipped_mismatch:
...
def present_status(entry):
    ...
    # File was found but RetroDECK has no expected MD5 — copied unverified
    if not expected or expected == []:
        return "Yes (unverified — no expected MD5 in RetroDECK manifests)"
...
if [[ ! "$CONFIRM_DEL_ZIP" =~ ^[Yy]$ && "$NO_HASH_COPY_COUNT" -gt 0 && -f "$ZIP_FILE" ]]; then
    ...
    echo "  These files matched the manifest by filename — RetroDECK has no expected MD5"
    echo "  for them. They are most likely valid but cannot be confirmed."
    echo "  Scrubbing them will remove them from rd_bios_set.zip only."
    echo "  Files already copied to staging or your live directory are not affected."
    echo ""
    echo -n "  Scrub unverified files from rd_bios_set.zip? (Y/N): "
